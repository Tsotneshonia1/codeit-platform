import { prisma } from "../lib/prisma"; // ყურადღება მიაქციე გზას!
import { revalidatePath } from "next/cache";

export default async function AdminDashboard() {
  // ბაზიდან ყველა დავალების წამოღება
  const assignments = await prisma.assignment.findMany({
    orderBy: {
      createdAt: "desc", // ახლები თავში
    },
    include: {
      student: true, // რომ სტუდენტის სახელიც ვნახოთ
    },
  });
                ...
                <tr key={task.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="p-5">
                    <span className={`px-3 py-1 rounded-full text-xs font-bold ${
                      task.status === "PENDING" ? "bg-amber-100 text-amber-700" : "bg-green-100 text-green-700"
                    }`}>
                      {task.status}
                    </span>
                  </td>
